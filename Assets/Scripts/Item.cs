...
public class Item : MonoBehaviour
{
    ...
    public ParticleSystem _pickupEffect; // 取得時のエフェクト（後で追加用）
    ...
    private void Awake()
    {
        ...
        if (_sphereCollider == null)
        {
            ...
            if (_sphereCollider == null)
            {
                Debug.LogError("SphereCollider がこの GameObject に見つかりません！");
            }
        }
    }
    ...
    private void ApplyBonus(TunkController tunkController)
    {
        if (tunkController != null)
        {
            ...
            {
                currentEffect = Instantiate(_pickupEffect, tunkController.transform.position + Vector3.zero, Quaternion.identity);
    ...
    {
        _sprite.enabled = false; // アイテムを非表示にする
        _sphereCollider.enabled = false; // コライダーを無効にする

        await UniTask.Delay(TimeSpan.FromSeconds(_itemRespawnDuration));// 休止時間を待つ

        _sprite.enabled = true; // アイテムを再表示する
        _sphereCollider.enabled = true; // コライダーを再度有効にする
